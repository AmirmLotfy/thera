...
```bash
FIREBASE_ADMIN_KEY='<service-account-json>' node scripts/seed-articles.mjs
```

Upserts three bilingual published articles (see `src/lib/blog/articles.ts`) into Firestore `articles`.
...
| `notifyTherapistApplicationDecision` | Firestore `therapistApplications/{id}` update | Email on approve/reject (backup to Vercel API) |
