...
export function useRouteGate(
  ...
  const { user, effectiveRole, loading, configured, profile } = useAuth();
    ...
    if (opts.requireRole && user) {
      ...
      if (!effectiveRole) {
        ...
        if (
          ...
          setRedirectTo("/dashboard/therapist/pending");
          return;
        }
        setRedirectTo("/auth/role");
        return;
      }
      if (!roles.includes(effectiveRole)) { setRedirectTo("/"); return; }
    }
    setRedirectTo(null);
  }, [configured, loading, user, effectiveRole, profile, opts.requireAuth, opts.requireVerified, opts.requireRole]);
