...
function BookingWizard() {
  ...
  const slotsQ = useTherapistSlotsLive(therapistId);
  ...
  const selectedChild = childrenQ.data?.find((c) => c.id === childId) ?? null;

  React.useEffect(() => {
    if (isParent && childrenQ.data?.length && !childId) {
      setChildId(childrenQ.data[0].id);
    }
  }, [isParent, childrenQ.data, childId]);
  ...
  async function lockBooking() {
    if (!slotId) return;
    if (isParent && !childId) {
      toast.error(locale === "ar" ? "اختر طفلاً للحجز" : "Select a child to book for");
      return;
    }
    setSubmitting(true);
    try {
      ...
      const res = await fetch("/api/booking/lock", {
        ...
        body: JSON.stringify({
          ...
          notes: answers.concern,
        }),
      });
      if (res.status === 409) {
        toast.error(locale === "ar" ? "هذا الموعد لم يعد متاحًا. اختر موعدًا آخر." : "This slot was just taken. Please pick another.");
        setSlotId(null);
        setStep(1);
        return;
      }
      ...
      toast.error(locale === "ar" ? "تعذّر حجز الموعد" : "Could not reserve this slot.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <SiteShell>
      <section className="relative mx-auto max-w-3xl px-5 pb-12 pt-hero-under-site-header md:px-8">
        <motion.div
          aria-hidden
        ...
        >
          ...
          {step === 1 && (
            <div className="space-y-5">
              {isParent && (childrenQ.data?.length ?? 0) > 0 && (
                <Choice label={locale === "ar" ? "الحجز لـ" : "Booking for"}>
                  {(childrenQ.data ?? []).map((c) => (
                    <button
                      key={c.id}
                      type="button"
                      onClick={() => setChildId(c.id)}
                      className={`rounded-full px-3 py-1.5 text-sm ${childId === c.id ? "bg-ink text-cream" : "border border-border bg-card"}`}
                    >{c.name}</button>
                  ))}
                </Choice>
              )}
          ...
          {step === 2 && (
            <div className="space-y-4 text-sm">
              ...
              {selectedChild ? (
                <Row label={locale === "ar" ? "الطفل" : "Child"} value={selectedChild.name} />
              ) : null}
              ...
              <div className="rounded-2xl bg-sky/40 p-4 text-sm text-ink">
                ...
                {locale === "ar" ? "ستدفع عبر InstaPay في الخطوة التالية ثم يراجع المعالج الإثبات." : "You'll pay via InstaPay next, then your therapist verifies the transfer."}
              </div>
