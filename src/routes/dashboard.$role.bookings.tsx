...
import { useMyBookingsLive } from "@/lib/queries/dashboard";
...
function BookingsPage() {
  ...
  const { data: bookings = [], loading: isLoading } = useMyBookingsLive(user?.uid, queryRole);
  ...
  return (
    <div className="flex flex-col gap-3 rounded-3xl border border-border/70 bg-card p-5 md:flex-row md:items-center md:justify-between">
      <div className="space-y-1">
        ...
        {booking.childName ? (
          <p className="text-xs text-ink-muted">{locale === "ar" ? `لـ ${booking.childName}` : `For ${booking.childName}`}</p>
        ) : null}
      ...
      <div className="flex flex-wrap gap-2">
        {past && role !== "therapist" && (
          <Link
            to="/book/$therapistId"
            params={{ therapistId: booking.therapistId }}
