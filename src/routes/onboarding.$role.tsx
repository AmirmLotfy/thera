...
import { consumePostLoginRedirect, peekPostLoginRedirect } from "@/lib/auth-redirect";
...
function OnboardingPage() {
  ...
  const [therapistStep1, setTherapistStep1] = React.useState<TherapistStep1>({
    ...
    bio: "", languages: ["en"], pricePerSession: "", currency: "EGP",
    cvFile: null, cvName: "",
  });
  ...
  async function finish() {
    ...
    if (role === "therapist" && !user.emailVerified) {
      setFinishError(locale === "ar" ? "يرجى تأكيد بريدك الإلكتروني قبل التقديم." : "Please verify your email before applying.");
      ...
      if (canWrite) {
        ...
        if (role === "therapist") {
          await setDoc(doc(db!, "therapistApplications", uid), {
            ...
            pricePerSession: majorToMinor(Number(therapistStep1.pricePerSession) || 0),
            currency: therapistStep1.currency || "EGP",
            status: "submitted",
            ...
            <>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label={locale === "ar" ? "السعر لكل جلسة" : "Price per session"}>
                  <input type="number" min={0} className="input" placeholder={locale === "ar" ? "٤٠٠" : "400"} value={therapistStep1.pricePerSession} onChange={(e) => setTherapistStep1((s) => ({ ...s, pricePerSession: e.target.value }))} />
                  <p className="mt-1 text-xs text-ink-muted">{locale === "ar" ? "بالجنيه المصري (ج.م) لكل جلسة" : "Amount in Egyptian pounds (EGP) per session"}</p>
                </Field>
