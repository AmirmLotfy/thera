...
import {
  sendResendEmail,
  therapistApprovedEmailAr,
  therapistApprovedEmailEn,
  therapistRejectedEmailAr,
  therapistRejectedEmailEn,
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (body.decision === "reject") {
          ...
          const appUrl = process.env.PUBLIC_SITE_URL?.replace(/\/$/, "") || "https://www.wethera.site";
          const email = (app.email as string | undefined) ?? (await auth.getUser(app.uid).catch(() => null))?.email;
          if (email) {
            const userSnap = await db.collection("users").doc(app.uid).get();
            const lang = userSnap.data()?.language === "ar" ? "ar" : "en";
            const tpl = lang === "ar"
              ? therapistRejectedEmailAr(appUrl, body.reason ?? null)
              : therapistRejectedEmailEn(appUrl, body.reason ?? null);
            await sendResendEmail({ to: email, ...tpl });
          }
        ...
        if (email) {
          const userSnap = await db.collection("users").doc(app.uid).get();
          const lang = userSnap.data()?.language === "ar" ? "ar" : "en";
          const tpl = lang === "ar" ? therapistApprovedEmailAr(appUrl) : therapistApprovedEmailEn(appUrl);
          await sendResendEmail({ to: email, ...tpl });
        }
