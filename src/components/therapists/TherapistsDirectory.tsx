...
import { priceLabel, PRICE_FILTER_EGP, priceFilterToMinor } from "@/lib/money";
...
export function TherapistsDirectory({ search, setSearch, variant = "marketing" }: TherapistsDirectoryProps) {
  ...
  const filters = React.useMemo(
    () => ({
      ...
      maxPrice: search.max ? priceFilterToMinor(search.max) : null,
    ...
    <div className="flex flex-col gap-8">
      ...
      <FilterBlock title={t.therapists.priceRange}>
        <div className="flex flex-wrap gap-2">
          ...
          />
          {PRICE_FILTER_EGP.map((p) => (
            <FilterChip
              key={p.maxEgp}
              active={search.max === p.maxEgp}
              onClick={() => updateSearch({ max: search.max === p.maxEgp ? undefined : p.maxEgp })}
    ...
    <>
      <section className={cn("relative overflow-hidden border-b border-border/40", app ? "bg-card/30" : "")}>
        ...
        <div className={cn("relative mx-auto max-w-7xl px-4 sm:px-5 md:px-8", app ? "pb-6 pt-5 md:pb-8 md:pt-6" : "pb-8 pt-hero-under-site-header md:pb-10")}>
          ...
          <h1
            className={cn(
              "hero-title mt-3 max-w-3xl text-balance font-display md:mt-4",
              app ? "text-2xl sm:text-3xl md:text-4xl" : "hero-title-xl text-4xl sm:text-5xl md:text-7xl leading-[1.05] tracking-tight",
            )}
          >
            {locale === "ar"
