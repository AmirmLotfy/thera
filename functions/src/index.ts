...
function therapistApprovedEmailEn(appUrl: string) {
  const html = `
  <div style="font-family:-apple-system,sans-serif;max-width:560px;margin:auto;color:#1F1B2E">
    <div style="background:#CDB4DB;padding:32px;border-radius:24px">
...
function therapistApprovedEmailAr(appUrl: string) {
  const html = `
  <div dir="rtl" style="font-family:Tajawal,Arial,sans-serif;max-width:560px;margin:auto;color:#1F1B2E">
    <div style="background:#CDB4DB;padding:32px;border-radius:24px">
...
function therapistRejectedEmailEn(appUrl: string, reason?: string | null) {
...
function therapistRejectedEmailAr(appUrl: string, reason?: string | null) {
...
export const notifyTherapistApplicationDecision = onDocumentUpdated(
  { document: "therapistApplications/{id}", secrets: [RESEND_API_KEY, EMAIL_FROM, APP_URL] },
  async (event) => {
    ...
    const email =
      next === "approved"
        ...
        : locale === "ar"
          ? therapistRejectedEmailAr(appUrl, (after.adminNote as string | undefined) ?? null)
          : therapistRejectedEmailEn(appUrl, (after.adminNote as string | undefined) ?? null);
